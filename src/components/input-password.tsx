...
const InputPassword = ({ label, isRequired, register, error }: PasswordInputProps) => {
  ...
  return (
    <div className='w-full'>
      <div className='relative w-full'>
        ...
        >
          <SvgIcon name={visible ? 'EyeOpen' : 'EyeClosed'} className={'w-5 h-5 stroke-[#d4d4d8] fill-[#27272a]'} />
        </button>
        <label
          htmlFor={label}
        ...
        >
          {label}
          {isRequired && <span className='absolute top-[1px] -right-[10px] text-[#F31260] text-[1.1em]'>*</span>}
        </label>
      </div>

      {error && <p className='text-red-500 text-[0.75rem] mt-1 transition-all duration-150'>{error}</p>}
    </div>
  );
};
