...
const OrderItem = ({ item, onClick, isSelected, isActive }: OrderItemProps) => {
  ...
  return (
    <>
      <li
        className={`relative grid items-center gap-5 border border-gray-300 rounded-[4px] shadow-sm py-2 px-9 bg-gray-100 overflow-hidden transition-all
    ${isActive ? 'grid-cols-[auto_100px_auto] max-w-[470px]' : 'grid-cols-[250px_auto_100px_120px_40px]'}`}
      >
        <span className={`text-gray-600 text-base ${isActive && 'hidden'}`}>
          {item.title}
        </span>
        ...
        <div className='flex items-center gap-5'>
          <IconButton
            ...
            className='w-10 h-10 rounded-full bg-[#E8E8E8] overflow-hidden border border-gray-400 hover:bg-[#DADADA]'
          >
        ...
        <div className={`text-left ${isActive && 'hidden'}`}>
        ...
        <IconButton
          ...
          className={`p-2 hover:bg-[#E8E8E8] rounded justify-self-end ${isActive && 'hidden'}`}
        >
          <SvgIcon
            name={'trash'}
        ...
        <div
          className={`flex justify-center items-center absolute top-0 right-0 h-full p-2 bg-gray-300 transition-transform duration-700 ease-in-out ${isSelected ? 'translate-x-0' : 'translate-x-full'}`}
        >
